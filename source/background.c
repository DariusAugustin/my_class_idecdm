...
int background_functions(
  ...
  if (pba->has_cdm == _TRUE_) {
     ...
     pvecback[pba->index_bg_rho_cdm] = pba->Omega0_cdm * pow(pba->H0,2) / pow(a,3) + pba->Omega0_fld * pow(pba->H0,2) / pow(a,3) * ( (pba->xhi_idecdm / (3* pba->w0_fld +pba->xhi_idecdm ) ) * (1- pow(a,-3*pba->w0_fld-pba->xhi_idecdm)));
  ...
  if (pba->has_fld == _TRUE_) {
    ...
    p_tot += (w_fld+pba->xhi_idecdm/3) * pvecback[pba->index_bg_rho_fld];
    dp_dloga += (a*dw_over_da-3*(1+w_fld+pba->xhi_idecdm/3)*(w_fld+pba->xhi_idecdm/3))*pvecback[pba->index_bg_rho_fld];
  }
...
int background_w_fld(
  ...
  switch (pba->fluid_equation_of_state) {
  case CLP:
    *integral_fld = (3.*((1.+pba->w0_fld+pba->wa_fld)+pba->xhi_idecdm)*log(1./a) + pba->wa_fld*(a-1.));
    break;
  case EDE:
...
int background_initial_conditions(
  ...
  */
  ...
  if (pba->has_fld == _TRUE_) {
    ...
    pvecback_integration[pba->index_bi_rho_fld] = rho_fld_today /pow(a,3*(1+pba->w0_fld)+pba->xhi_idecdm);
...
int background_derivs(
  ...
  if (pba->has_fld == _TRUE_) {
    ...
    dy[pba->index_bi_rho_fld] = -(3.*(1.+pvecback[pba->index_bg_w_fld])+pba->xhi_idecdm)*y[pba->index_bi_rho_fld];
  }
