    ...
    for (index_k=0; index_k<ppt->k_size[index_md]; index_k++) {
      ...
      if (output_format == class_format) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          class_store_double(dataptr,tk[ppt->index_tp_delta_icdm],ppt->has_source_delta_icdm,storeidx);
          class_store_double(dataptr,tk[ppt->index_tp_delta_idm],ppt->has_source_delta_idm,storeidx);
          class_store_double(dataptr,tk[ppt->index_tp_delta_idefld],ppt->has_source_delta_idefld,storeidx);
          ...
          if (pba->has_nicdm == _TRUE_){
            for (n_nicdm = 0; n_nicdm < pba->N_nicdm; n_nicdm++){
              class_store_double(dataptr,tk[ppt->index_tp_delta_nicdm1+n_nicdm],ppt->has_source_delta_nicdm,storeidx);
            }
          }
          class_store_double(dataptr,tk[ppt->index_tp_delta_dicdm],ppt->has_source_delta_dicdm,storeidx);
        ...
        if (ppt->has_velocity_transfers == _TRUE_) {
          ...
          class_store_double(dataptr,tk[ppt->index_tp_theta_icdm],ppt->has_source_theta_icdm,storeidx);
          class_store_double(dataptr,tk[ppt->index_tp_theta_idm],ppt->has_source_theta_idm,storeidx);
          class_store_double(dataptr,tk[ppt->index_tp_theta_idefld],ppt->has_source_theta_idefld,storeidx);
          ...
          if (pba->has_nicdm == _TRUE_){
            for (n_nicdm = 0; n_nicdm < pba->N_nicdm; n_nicdm++){
              class_store_double(dataptr,tk[ppt->index_tp_theta_nicdm1+n_nicdm],ppt->has_source_theta_nicdm,storeidx);
            }
          }
          class_store_double(dataptr,tk[ppt->index_tp_theta_dicdm],ppt->has_source_theta_dicdm,storeidx);
        ...
        class_store_double_or_default(dataptr,-tk[ppt->index_tp_delta_icdm]/k2,ppt->has_source_delta_icdm,storeidx,0.0);
        ...
        class_store_double_or_default(dataptr,-tk[ppt->index_tp_delta_nicdm1]/k2,ppt->has_source_delta_nicdm,storeidx,0.0);
...
int perturbations_output_titles(
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"d_icdm",pba->has_icdm);
      class_store_columntitle(titles,"d_idm",pba->has_idm);
      class_store_columntitle(titles,"d_idefld",pba->has_idefld);
      class_store_columntitle(titles,"d_ur",pba->has_ur);
      class_store_columntitle(titles,"d_idr",pba->has_idr);
      if (pba->has_nicdm == _TRUE_) {
        for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++) {
          sprintf(tmp,"d_nicdm[%d]",n_nicdm);
          class_store_columntitle(titles,tmp,_TRUE_);
        }
      }
      class_store_columntitle(titles,"d_dicdm",pba->has_dicdm);
    ...
    if (ppt->has_velocity_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"t_icdm",((pba->has_icdm == _TRUE_) && (ppt->gauge != synchronous)));
      class_store_columntitle(titles,"t_idm",pba->has_idm);
      class_store_columntitle(titles,"t_idefld",pba->has_idefld);
      class_store_columntitle(titles,"t_ur",pba->has_ur);
      class_store_columntitle(titles,"t_idr",pba->has_idr);
      if (pba->has_nicdm == _TRUE_) {
        for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++) {
          sprintf(tmp,"t_nicdm[%d]",n_nicdm);
          class_store_columntitle(titles,tmp,_TRUE_);
        }
      }
      class_store_columntitle(titles,"t_dicdm",pba->has_dicdm);
    ...
    class_store_columntitle(titles,"k (h/Mpc)",_TRUE_);
    class_store_columntitle(titles,"-T_icdm/k2",_TRUE_);
    ...
    class_store_columntitle(titles,"-T_nicdm/k2",_TRUE_);
...
int perturbations_init(
  ...
  double w_idefld_ini, w_idefld_0,dw_over_da_idefld,integral_idefld;
  ...
  class_test((ppt->gauge == synchronous) && (pba->has_icdm == _FALSE_),
             ppt->error_message,
  ...
  if (pba->has_nicdm == _TRUE_) {
    ...
    class_test ((ppr->nicdm_fluid_approximation < nicdmfa_mb) ||
                (ppr->nicdm_fluid_approximation > nicdmfa_none),
                ppt->error_message,
                "your nicdm_fluid_approximation is set to %d, out of range defined in perturbations.h",ppr->nicdm_fluid_approximation);
  ...
  if (pba->has_idefld == _TRUE_) {

    /* check values of w_idefld at initial time and today. Since 'a' in the code stands for 'a/a_0', its current value is 1 by definition */
    class_call(background_w_idefld(pba, 0., &w_idefld_ini, &dw_over_da_idefld, &integral_idefld), pba->error_message, ppt->error_message);
    class_call(background_w_idefld(pba, 1.,   &w_idefld_0, &dw_over_da_idefld, &integral_idefld), pba->error_message, ppt->error_message);

    class_test(w_idefld_ini >= 0.,
               ppt->error_message,
               "The fluid is meant to be negligible at early time, and unimportant for defining the initial conditions of other species. You are using parameters for which this assumption may break down, since at early times you have w_idefld(a--->0) = %e >= 0",w_idefld_ini);

    if (pba->use_ppf == _FALSE_) {

      class_test((w_idefld_ini +1.0)*(w_idefld_0+1.0) <= 0.0,
                 ppt->error_message,
      ...
      class_test((w_idefld_0 == -1.) && (dw_over_da_idefld == 0.),
                 ppt->error_message,
  ...
  if (ppt->has_tensors == _TRUE_) {
    ...
    ppt->evolve_tensor_nicdm = _FALSE_;
    ...
    case (tm_massless_approximation):
      if ((pba->has_ur == _TRUE_) || (pba->has_nicdm == _TRUE_))
    ...
    case (tm_exact):
      if (pba->has_ur == _TRUE_)
        ppt->evolve_tensor_ur = _TRUE_;
      if (pba->has_nicdm == _TRUE_)
        ppt->evolve_tensor_nicdm = _TRUE_;
      break;
    }
  }
...
int perturbations_indices(
  ...
  ppt->has_source_delta_icdm = _FALSE_;
  ppt->has_source_delta_idm = _FALSE_;
  ppt->has_source_delta_dicdm = _FALSE_;
  ppt->has_source_delta_idefld = _FALSE_;
  ...
  ppt->has_source_delta_nicdm = _FALSE_;
  ppt->has_source_delta_tot = _FALSE_;
  ...
  ppt->has_source_theta_icdm = _FALSE_;
  ppt->has_source_theta_idm = _FALSE_;
  ppt->has_source_theta_dicdm = _FALSE_;
  ppt->has_source_theta_idefld = _FALSE_;
  ...
  ppt->has_source_theta_nicdm = _FALSE_;
  ppt->has_source_theta_tot = _FALSE_;
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        if (pba->has_icdm == _TRUE_)
          ppt->has_source_delta_icdm = _TRUE_;
        if (pba->has_idm == _TRUE_)
          ppt->has_source_delta_idm = _TRUE_;
        if (pba->has_dicdm == _TRUE_)
          ppt->has_source_delta_dicdm = _TRUE_;
        if (pba->has_idefld == _TRUE_)
          ppt->has_source_delta_idefld = _TRUE_;
        ...
        if (pba->has_nicdm == _TRUE_)
          ppt->has_source_delta_nicdm = _TRUE_;
      ...
      if (ppt->has_velocity_transfers == _TRUE_) {
        ...
        if ((pba->has_icdm == _TRUE_) && (ppt->gauge != synchronous))
          ppt->has_source_theta_icdm = _TRUE_;
        if (pba->has_idm == _TRUE_)
          ppt->has_source_theta_idm = _TRUE_;
        if (pba->has_dicdm == _TRUE_)
          ppt->has_source_theta_dicdm = _TRUE_;
        if (pba->has_idefld == _TRUE_)
          ppt->has_source_theta_idefld = _TRUE_;
        if (pba->has_scf == _TRUE_)
        ...
        if (pba->has_nicdm == _TRUE_)
          ppt->has_source_theta_nicdm = _TRUE_;
      }
      ...
      class_define_index(ppt->index_tp_delta_icdm,  ppt->has_source_delta_icdm, index_type,1);
      class_define_index(ppt->index_tp_delta_idm,  ppt->has_source_delta_idm, index_type,1);
      class_define_index(ppt->index_tp_delta_dicdm, ppt->has_source_delta_dicdm,index_type,1);
      class_define_index(ppt->index_tp_delta_idefld,  ppt->has_source_delta_idefld, index_type,1);
      ...
      class_define_index(ppt->index_tp_delta_nicdm1,ppt->has_source_delta_nicdm,index_type,pba->N_nicdm);
      ...
      class_define_index(ppt->index_tp_theta_icdm,  ppt->has_source_theta_icdm, index_type,1);
      class_define_index(ppt->index_tp_theta_idm,  ppt->has_source_theta_idm, index_type,1);
      class_define_index(ppt->index_tp_theta_dicdm, ppt->has_source_theta_dicdm,index_type,1);
      class_define_index(ppt->index_tp_theta_idefld,  ppt->has_source_theta_idefld, index_type,1);
      ...
      class_define_index(ppt->index_tp_theta_nicdm1,ppt->has_source_theta_nicdm,index_type,pba->N_nicdm);
...
int perturbations_workspace_init(
  ...
  if (_scalars_) {
    ...
    if (pba->has_nicdm == _TRUE_) ppw->max_l_max = MAX(ppw->max_l_max, ppr->l_max_nicdm);
  ...
  if (_scalars_) {
    ...
    class_define_index(ppw->index_ap_nicdmfa,pba->has_nicdm,index_ap,1);
  ...
  if (_scalars_) {
    ...
    if (pba->has_nicdm == _TRUE_) {
      ppw->approx[ppw->index_ap_nicdmfa]=(int)nicdmfa_off;
    }
  }
  ...
  if (_scalars_) {
    ...
    if ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_)) {
      ...
      class_alloc(ppw->delta_nicdm,pba->N_nicdm*sizeof(double),ppt->error_message);
      class_alloc(ppw->theta_nicdm,pba->N_nicdm*sizeof(double),ppt->error_message);
      class_alloc(ppw->shear_nicdm,pba->N_nicdm*sizeof(double),ppt->error_message);
...
int perturbations_workspace_free (
  ...
  if (_scalars_) {
    ...
    if ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_)) {
      free(ppw->delta_nicdm);
      free(ppw->theta_nicdm);
      free(ppw->shear_nicdm);
    }
  }
...
int perturbations_solve(
  ...
  if (pba->has_nicdm == _TRUE_) {
    for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++) {
      class_test(fabs(ppw->pvecback[pba->index_bg_p_nicdm1+n_nicdm]/ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm]-1./3.)>ppr->tol_nicdm_initial_w,
                 ppt->error_message,
                 "your choice of initial time for integrating wavenumbers is inappropriate: it corresponds to a time at which the nicdm species number %d is not ultra-relativistic anymore, with w=%g, p=%g and rho=%g\n",
                 n_nicdm,
                 ppw->pvecback[pba->index_bg_p_nicdm1+n_nicdm]/ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm],
                 ppw->pvecback[pba->index_bg_p_nicdm1+n_nicdm],
                 ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm]);
    }
  }
  ...
  while ((tau_upper - tau_lower)/tau_lower > ppr->tol_tau_approx) {
    ...
    if (pba->has_nicdm == _TRUE_) {
      for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++) {
        if (fabs(ppw->pvecback[pba->index_bg_p_nicdm1+n_nicdm]/ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm]-1./3.) > ppr->tol_nicdm_initial_w)
...
int perturbations_prepare_k_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      /* Cold dark matter */
      class_store_columntitle(ppt->scalar_titles,"delta_icdm",pba->has_icdm);
      ...
      if ((pba->has_nicdm == _TRUE_) && ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_))) {
        for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++){
          sprintf(tmp,"delta_nicdm[%d]",n_nicdm);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
          sprintf(tmp,"theta_nicdm[%d]",n_nicdm);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
          sprintf(tmp,"shear_nicdm[%d]",n_nicdm);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
          sprintf(tmp,"cs2_nicdm[%d]",n_nicdm);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
        }
      }
      /* Decaying cold dark matter */
      class_store_columntitle(ppt->scalar_titles, "delta_dicdm", pba->has_dicdm);
      class_store_columntitle(ppt->scalar_titles, "theta_dicdm", pba->has_dicdm);
      /* Decay radiation */
      ...
      /** Fluid */
      class_store_columntitle(ppt->scalar_titles, "delta_rho_idefld", pba->has_idefld);
      class_store_columntitle(ppt->scalar_titles, "rho_plus_p_theta_idefld", pba->has_idefld);
      class_store_columntitle(ppt->scalar_titles, "delta_p_idefld", pba->has_idefld);
    ...
    if (ppt->has_tensors == _TRUE_){
      ...
      if (ppt->evolve_tensor_nicdm == _TRUE_) {
        for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++){
          sprintf(tmp,"delta_nicdm[%d]",n_nicdm);
    ...
    for (index_switch=1; index_switch<interval_number; index_switch++) {
      ...
      if (ppt->perturbations_verbose>2) {
        ...
        if (_scalars_) {
          ...
          if (pba->has_nicdm == _TRUE_) {
            if ((interval_approx[index_switch-1][ppw->index_ap_nicdmfa]==(int)nicdmfa_off) &&
                (interval_approx[index_switch][ppw->index_ap_nicdmfa]==(int)nicdmfa_on)) {
              fprintf(stdout,"Mode k=%e: will switch on nicdm fluid approximation at tau=%e\n",k,interval_limit[index_switch]);
            }
          }
        }
...
int perturbations_vector_init(
  ...
  int index_pt;
  int l;
  int n_nicdm,index_q,nicdm_l_size;
  double rho_plus_p_nicdm,q,q2,epsilon,a,factor;
  ...
  ppv->l_max_nicdm = NULL;
  ppv->q_size_nicdm = NULL;
  ...
  if (_scalars_) {
    ...
    class_define_index(ppv->index_pt_theta_icdm,pba->has_icdm && (ppt->gauge == newtonian),index_pt,1); /* icdm velocity */
    ...
    class_define_index(ppv->index_pt_delta_dicdm,pba->has_dicdm,index_pt,1); /* dicdm density */
    class_define_index(ppv->index_pt_theta_dicdm,pba->has_dicdm,index_pt,1); /* dicdm velocity */
    ...
    if (pba->use_ppf == _FALSE_) {
      class_define_index(ppv->index_pt_delta_idefld,pba->has_idefld,index_pt,1); /* fluid density */
      class_define_index(ppv->index_pt_theta_idefld,pba->has_idefld,index_pt,1); /* fluid velocity */
    }
    else {
      class_define_index(ppv->index_pt_Gamma_idefld,pba->has_idefld,index_pt,1); /* Gamma variable of PPF scheme */
    }
    ...
    if (pba->has_nicdm == _TRUE_) {
      ppv->index_pt_psi0_nicdm1 = index_pt; /* density of ultra-relativistic neutrinos/relics */
      ppv->N_nicdm = pba->N_nicdm;
      class_alloc(ppv->l_max_nicdm,ppv->N_nicdm*sizeof(double),ppt->error_message);
      class_alloc(ppv->q_size_nicdm,ppv->N_nicdm*sizeof(double),ppt->error_message);

      for (n_nicdm = 0; n_nicdm < pba->N_nicdm; n_nicdm++){
        // Set value of ppv->l_max_nicdm:
        if (ppw->approx[ppw->index_ap_nicdmfa] == (int)nicdmfa_off){
          /* reject inconsistent values of the number of mutipoles in ultra relativistic neutrino hierarchy */
          class_test(ppr->l_max_nicdm < 4,
                     ppt->error_message,
                     "ppr->l_max_nicdm=%d should be at least 4, i.e. we must integrate at least over first four momenta of non-cold dark matter perturbed phase-space distribution",n_nicdm);
          //Copy value from precision parameter:
          ppv->l_max_nicdm[n_nicdm] = ppr->l_max_nicdm;
          ppv->q_size_nicdm[n_nicdm] = pba->q_size_nicdm[n_nicdm];
        }
        else{
          // In the fluid approximation, hierarchy is cut at lmax = 2 and q dependence is integrated out:
          ppv->l_max_nicdm[n_nicdm] = 2;
          ppv->q_size_nicdm[n_nicdm] = 1;
        }
        index_pt += (ppv->l_max_nicdm[n_nicdm]+1)*ppv->q_size_nicdm[n_nicdm];
      }
    }
  ...
  if (_tensors_) {
    ...
    if (ppt->evolve_tensor_nicdm == _TRUE_) {
      ppv->index_pt_psi0_nicdm1 = index_pt;
      ppv->N_nicdm = pba->N_nicdm;
      class_alloc(ppv->l_max_nicdm,ppv->N_nicdm*sizeof(double),ppt->error_message);
      class_alloc(ppv->q_size_nicdm,ppv->N_nicdm*sizeof(double),ppt->error_message);
  ...
  if (_scalars_) {
    ...
    if (pba->has_nicdm == _TRUE_) {
      ...
      /* we don't need nicdm multipoles above l=2 (but they are
         defined only when nicdmfa is off) */

      index_pt = ppv->index_pt_psi0_nicdm1;
      for (n_nicdm = 0; n_nicdm < ppv-> N_nicdm; n_nicdm++){
        for (index_q=0; index_q < ppv->q_size_nicdm[n_nicdm]; index_q++){
          for (l=0; l<=ppv->l_max_nicdm[n_nicdm]; l++){
  ...
  if (pa_old == NULL) {
    ...
    if (_scalars_) {
      ...
      if (pba->has_nicdm == _TRUE_) {
        ...
        class_test(ppw->approx[ppw->index_ap_nicdmfa] == (int)nicdmfa_on,
                   ppt->error_message,
                   "scalar initial conditions assume nicdm fluid approximation turned off");
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      /** - ---> (a.2.) some variables (b, icdm, idefld, ...) are not affected by
      ...
      if (pba->has_dicdm == _TRUE_) {
        ...
        ppv->y[ppv->index_pt_delta_dicdm] =
          ppw->pv->y[ppw->pv->index_pt_delta_dicdm];

        ppv->y[ppv->index_pt_theta_dicdm] =
          ppw->pv->y[ppw->pv->index_pt_theta_dicdm];
      }
      ...
      if (pba->has_idefld == _TRUE_) {
        ...
        if (pba->use_ppf == _FALSE_) {
          ppv->y[ppv->index_pt_delta_idefld] =
            ppw->pv->y[ppw->pv->index_pt_delta_idefld];

          ppv->y[ppv->index_pt_theta_idefld] =
            ppw->pv->y[ppw->pv->index_pt_theta_idefld];
        }
        else {
          ppv->y[ppv->index_pt_Gamma_idefld] =
            ppw->pv->y[ppw->pv->index_pt_Gamma_idefld];
        }
      }
      ...
      if ((pa_old[ppw->index_ap_tca] == (int)tca_on) && (ppw->approx[ppw->index_ap_tca] == (int)tca_off)) {
        ...
        if (pba->has_nicdm == _TRUE_) {
          ...
          for (n_nicdm = 0; n_nicdm < ppv->N_nicdm; n_nicdm++){
            for (index_q=0; index_q < ppv->q_size_nicdm[n_nicdm]; index_q++){
              for (l=0; l<=ppv->l_max_nicdm[n_nicdm];l++){
                // This is correct with or without nicdmfa, since ppv->lmax_nicdm is set accordingly.
                ppv->y[ppv->index_pt_psi0_nicdm1+index_pt] =
                  ppw->pv->y[ppw->pv->index_pt_psi0_nicdm1+index_pt];
                index_pt++;
              }
            }
          }
        ...
        if ((pa_old[ppw->index_ap_ufa] == (int)ufa_off) && (ppw->approx[ppw->index_ap_ufa] == (int)ufa_on)) {
          ...
          if (pba->has_nicdm == _TRUE_) {
            ...
            for (n_nicdm = 0; n_nicdm < ppv->N_nicdm; n_nicdm++){
              for (index_q=0; index_q < ppv->q_size_nicdm[n_nicdm]; index_q++){
                for (l=0; l<=ppv->l_max_nicdm[n_nicdm]; l++){
                  /* This is correct even when nicdmfa == off, since ppv->l_max_nicdm and
                     ppv->q_size_nicdm is updated.*/
                  ppv->y[ppv->index_pt_psi0_nicdm1+index_pt] =
                    ppw->pv->y[ppw->pv->index_pt_psi0_nicdm1+index_pt];
                  index_pt++;
                }
      ...
      /* -- case of switching on nicdm fluid
         approximation. Provide correct initial conditions to new set
         of variables */

      if (pba->has_nicdm == _TRUE_) {

        if ((pa_old[ppw->index_ap_nicdmfa] == (int)nicdmfa_off) && (ppw->approx[ppw->index_ap_nicdmfa] == (int)nicdmfa_on)) {

          if (ppt->perturbations_verbose>2)
            fprintf(stdout,"Mode k=%e: switch on nicdm fluid approximation at tau=%e\n",k,tau);
          ...
          index_pt = ppw->pv->index_pt_psi0_nicdm1;
          for (n_nicdm = 0; n_nicdm < ppv->N_nicdm; n_nicdm++){
            // We are in the fluid approximation, so nicdm_l_size is always 3.
            nicdm_l_size = ppv->l_max_nicdm[n_nicdm]+1;
            rho_plus_p_nicdm = ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm]+
              ppw->pvecback[pba->index_bg_p_nicdm1+n_nicdm];
            for (l=0; l<=2; l++){
              ppv->y[ppv->index_pt_psi0_nicdm1+nicdm_l_size*n_nicdm+l] = 0.0;
            }
            factor = pba->factor_nicdm[n_nicdm]/pow(a,4);
            for (index_q=0; index_q < ppw->pv->q_size_nicdm[n_nicdm]; index_q++){
              // Integrate over distributions:
              q = pba->q_nicdm[n_nicdm][index_q];
              q2 = q*q;
              epsilon = sqrt(q2+a*a*pba->M_nicdm[n_nicdm]*pba->M_nicdm[n_nicdm]);
              ppv->y[ppv->index_pt_psi0_nicdm1+nicdm_l_size*n_nicdm] +=
                pba->w_nicdm[n_nicdm][index_q]*q2*epsilon*
                ppw->pv->y[index_pt];

              ppv->y[ppv->index_pt_psi0_nicdm1+nicdm_l_size*n_nicdm+1] +=
                pba->w_nicdm[n_nicdm][index_q]*q2*q*
                ppw->pv->y[index_pt+1];

              ppv->y[ppv->index_pt_psi0_nicdm1+nicdm_l_size*n_nicdm+2] +=
                pba->w_nicdm[n_nicdm][index_q]*q2*q2/epsilon*
                ppw->pv->y[index_pt+2];

              //Jump to next momentum bin in ppw->pv->y:
              index_pt += (ppw->pv->l_max_nicdm[n_nicdm]+1);
            }
            ppv->y[ppv->index_pt_psi0_nicdm1+nicdm_l_size*n_nicdm] *=factor/ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm];
            ppv->y[ppv->index_pt_psi0_nicdm1+nicdm_l_size*n_nicdm+1] *=k*factor/rho_plus_p_nicdm;
            ppv->y[ppv->index_pt_psi0_nicdm1+nicdm_l_size*n_nicdm+2] *=2.0/3.0*factor/rho_plus_p_nicdm;
          }
        }
      }
    }
...
int perturbations_vector_free(
  ...
  if (pv->l_max_nicdm != NULL) free(pv->l_max_nicdm);
  if (pv->q_size_nicdm != NULL) free(pv->q_size_nicdm);
  free(pv->y);
  free(pv->dy);
...
int perturbations_initial_conditions(struct precision * ppr,
  ...
  double w_idefld,dw_over_da_idefld,integral_idefld;
  double delta_ur=0.,theta_ur=0.,shear_ur=0.,l3_ur=0.,eta=0.,delta_icdm=0.,alpha, alpha_prime;
  double delta_dr=0;
  double q,epsilon,k2;
  int index_q,n_nicdm,idx;
  double rho_r,rho_m,rho_nu,rho_m_over_rho_r, rho_icdm =0.;
  double fracnu,fracg,fracb,fracicdm = 0.,fracidm = 0.;
  double om;
  ...
  if (_scalars_) {
    ...
    if (pba->has_dicdm == _TRUE_) {
      rho_m += ppw->pvecback[pba->index_bg_rho_dicdm];
    }
    ...
    if (pba->has_nicdm == _TRUE_) {
      for (n_nicdm=0; n_nicdm<pba->N_nicdm; n_nicdm++){
        rho_r += ppw->pvecback[pba->index_bg_rho_nicdm1 + n_nicdm];
        rho_nu += ppw->pvecback[pba->index_bg_rho_nicdm1 + n_nicdm];
      }
    }
    ...
    /* f_icdm = Omega_icdm(t_i) / Omega_m(t_i) */
    if (pba->has_icdm == _TRUE_)
      fracicdm = ppw->pvecback[pba->index_bg_rho_icdm]/rho_m;
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      if (pba->has_icdm == _TRUE_) {
        ...
        /* icdm velocity vanishes in the synchronous gauge */
      }
      ...
      if (pba->has_dicdm == _TRUE_) {
        ppw->pv->y[ppw->pv->index_pt_delta_dicdm] = 3./4.*ppw->pv->y[ppw->pv->index_pt_delta_g]; /* dicdm density */
        /* dicdm velocity velocity vanishes initially in the synchronous gauge */
      ...
      if (pba->has_idefld == _TRUE_) {
        ...
        class_call(background_w_idefld(pba,a,&w_idefld,&dw_over_da_idefld,&integral_idefld), pba->error_message, ppt->error_message);

        if (pba->use_ppf == _FALSE_) {
          ppw->pv->y[ppw->pv->index_pt_delta_idefld] = - ktau_two/4.*(1.+w_idefld)*(4.-3.*pba->cs2_idefld)/(4.-6.*w_idefld+3.*pba->cs2_idefld) * ppr->curvature_ini * s2_squared; /* from 1004.5509 */ //TBC: curvature

          ppw->pv->y[ppw->pv->index_pt_theta_idefld] = - k*ktau_three/4.*pba->cs2_idefld/(4.-6.*w_idefld+3.*pba->cs2_idefld) * ppr->curvature_ini * s2_squared; /* from 1004.5509 */ //TBC:curvature
        }
        /* if use_ppf == _TRUE_, y[ppw->pv->index_pt_Gamma_idefld] will be automatically set to zero, and this is what we want (although one could probably work out some small nonzero initial conditions: TODO) */
      }
      ...
      if (pba->has_scf == _TRUE_) {
        ...
        /* delta_idefld expression * rho_scf with the w = 1/3, c_s = 1
      ...
      /* all relativistic relics: ur, early nicdm, dr */

      if ((pba->has_ur == _TRUE_) || (pba->has_nicdm == _TRUE_) || (pba->has_dr == _TRUE_) || (pba->has_idr == _TRUE_)) {
    ...
    if ((ppt->has_cdi == _TRUE_) && (index_ic == ppt->index_ic_cdi)) {
      ...
      class_test(pba->has_icdm == _FALSE_,
                 ppt->error_message,
      ...
      ppw->pv->y[ppw->pv->index_pt_delta_g] = ppr->entropy_ini*fracicdm*om*tau*(-2./3.+om*tau/4.);
      ppw->pv->y[ppw->pv->index_pt_theta_g] = -ppr->entropy_ini*fracicdm*om*ktau_two/12.;
      ...
      if ((pba->has_ur == _TRUE_) || (pba->has_nicdm == _TRUE_)) {
        ...
        shear_ur = -ppr->entropy_ini*fracicdm*ktau_two*tau*om/6./(2.*fracnu+15.);

      }

      eta = -ppr->entropy_ini*fracicdm*om*tau*(1./6.-om*tau/16.);
    ...
    if ((ppt->has_nid == _TRUE_) && (index_ic == ppt->index_ic_nid)) {
      ...
      class_test((pba->has_ur == _FALSE_) && (pba->has_nicdm == _FALSE_),
                 ppt->error_message,
                 "not consistent to ask for NID in absence of ur or nicdm species!");
    ...
    if ((ppt->has_niv == _TRUE_) && (index_ic == ppt->index_ic_niv)) {
      ...
      class_test((pba->has_ur == _FALSE_) && (pba->has_nicdm == _FALSE_),
                 ppt->error_message,
                 "not consistent to ask for NIV in absence of ur or nicdm species!");
    ...
    if (ppt->gauge == newtonian) {
      ...
      /* alpha is like in Ma & Bertschinger: (h'+6 eta')/(2k^2). We obtain it from the first two Einstein equations:
         ...
         = [(f_g delta_g + f_nu delta_nu) + (rho_m/rho_r) (f_b delta_b + f_icdm delta_icdm)] / (1 + rho_m/rho_r)

         velocity_tot = ((rho+p)theta/rho_c)
         = [(4/3) rho_r theta_r + rho_m theta_m] / (rho_r + rho_m)
         = [(4/3) theta_r + (rho_m/rho_r) theta_m] / (1 + rho_m/rho_r)
         = [(4/3) (f_g theta_g + f_nu theta_nu) + (rho_m/rho_r) (f_b delta_b + f_icdm 0)] / (1 + rho_m/rho_r)
      */

      if (pba->has_icdm == _TRUE_) {
        delta_icdm += ppw->pvecback[pba->index_bg_rho_icdm] * ppw->pv->y[ppw->pv->index_pt_delta_icdm];
        rho_icdm += ppw->pvecback[pba->index_bg_rho_icdm];
      }
      if (pba->has_idm == _TRUE_) {
        delta_icdm += ppw->pvecback[pba->index_bg_rho_idm] * ppw->pv->y[ppw->pv->index_pt_delta_idm];
        rho_icdm += ppw->pvecback[pba->index_bg_rho_idm];
      }
      if (pba->has_dicdm == _TRUE_){
        delta_icdm += ppw->pvecback[pba->index_bg_rho_dicdm] * ppw->pv->y[ppw->pv->index_pt_delta_dicdm];
        rho_icdm += ppw->pvecback[pba->index_bg_rho_dicdm];
      }


      if (rho_icdm > 0 ) {
        delta_icdm /= rho_icdm;
        fracicdm = rho_icdm/rho_m;
      }

      // note: if there are no neutrinos, fracnu, delta_ur and theta_ur below will consistently be zero.

      delta_tot = (fracg*ppw->pv->y[ppw->pv->index_pt_delta_g]+fracnu*delta_ur+rho_m_over_rho_r*(fracb*ppw->pv->y[ppw->pv->index_pt_delta_b]+fracicdm*delta_icdm))/(1.+rho_m_over_rho_r);
      ...
      if (pba->has_dicdm == _TRUE_) {
        ppw->pv->y[ppw->pv->index_pt_delta_dicdm] -= (3.*a_prime_over_a + a*pba->Gamma_dicdm)*alpha;
        ppw->pv->y[ppw->pv->index_pt_theta_dicdm] = k*k*alpha;
      }

      /* fluid */
      if ((pba->has_idefld == _TRUE_) && (pba->use_ppf == _FALSE_)) {

        class_call(background_w_idefld(pba,a,&w_idefld,&dw_over_da_idefld,&integral_idefld), pba->error_message, ppt->error_message);

        ppw->pv->y[ppw->pv->index_pt_delta_idefld] -= 3*(1.+w_idefld)*a_prime_over_a*alpha;
        ppw->pv->y[ppw->pv->index_pt_theta_idefld] += k*k*alpha;
      }
      ...
      if ((pba->has_ur == _TRUE_) || (pba->has_nicdm == _TRUE_) || (pba->has_dr == _TRUE_)  || (pba->has_idr == _TRUE_)) {
        ...
        if (pba->has_dr == _TRUE_)
          delta_dr += (-4.*a_prime_over_a + a*pba->Gamma_dicdm*ppw->pvecback[pba->index_bg_rho_dicdm]/ppw->pvecback[pba->index_bg_rho_dr])*alpha;

      }

    } /* end of gauge transformation to newtonian gauge */

      /** - (e) In any gauge, we should now implement the relativistic initial conditions in ur and nicdm variables */
    ...
    if (pba->has_nicdm == _TRUE_) {
      idx = ppw->pv->index_pt_psi0_nicdm1;
      for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++){
        ...
        for (index_q=0; index_q < ppw->pv->q_size_nicdm[n_nicdm]; index_q++) {
          ...
          epsilon = sqrt(q*q+a*a*pba->M_nicdm[n_nicdm]*pba->M_nicdm[n_nicdm]);

          ppw->pv->y[idx] = -0.25 * delta_ur * pba->dlnf0_dlnq_nicdm[n_nicdm][index_q];

          ppw->pv->y[idx+1] =  -epsilon/3./q/k*theta_ur* pba->dlnf0_dlnq_nicdm[n_nicdm][index_q];

          ppw->pv->y[idx+2] = -0.5 * shear_ur * pba->dlnf0_dlnq_nicdm[n_nicdm][index_q];

          ppw->pv->y[idx+3] = -0.25 * l3_ur * pba->dlnf0_dlnq_nicdm[n_nicdm][index_q];

          //Jump to next momentum bin:
          idx += (ppw->pv->l_max_nicdm[n_nicdm]+1);
...
int perturbations_approximations(
  ...
  if (_scalars_) {
    ...
    if (pba->has_nicdm == _TRUE_) {
      ...
      if ((tau/tau_k > ppr->nicdm_fluid_trigger_tau_over_tau_k) &&
          (ppr->nicdm_fluid_approximation != nicdmfa_none)) {

        ppw->approx[ppw->index_ap_nicdmfa] = (int)nicdmfa_on;
      }
      else {
        ppw->approx[ppw->index_ap_nicdmfa] = (int)nicdmfa_off;
      }
...
int perturbations_timescale(
  ...
  if ((ppt->has_scalars == _TRUE_) && (pppaw->index_md == ppt->index_md_scalars)) {
    ...
    *timescale = tau_h;
    ...
    if ((ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) || (pba->has_nicdm == _TRUE_))
...
int perturbations_total_stress_energy(
  ...
  double theta_idr=0.;
  double shear_idr=0.;
  double rho_delta_nicdm=0.;
  double rho_plus_p_theta_nicdm=0.;
  double rho_plus_p_shear_nicdm=0.;
  double delta_p_nicdm=0.;
  double factor;
  double rho_plus_p_nicdm;
  int index_q,n_nicdm,idx;
  double epsilon,q,q2,cg2_nicdm,w_nicdm,rho_nicdm_bg,p_nicdm_bg,pseudo_p_nicdm;
  double w_idefld,dw_over_da_idefld,integral_idefld;
  double gwnicdm;
  ...
  double w_prime_idefld, ca2_idefld;
  double alpha, alpha_prime, metric_euler;
  double rho_t, p_t, rho_t_prime, p_t_prime;
  double rho_idefld, p_idefld, rho_idefld_prime, p_idefld_prime;
  double X, Y, Z, X_prime, Y_prime, Z_prime;
  double Gamma_idefld, S, S_prime, theta_t, theta_t_prime, rho_plus_p_theta_idefld_prime;
  ...
  if (_scalars_) {
    ...
    /* icdm contribution */
    if (pba->has_icdm == _TRUE_) {
      ppw->delta_rho += ppw->pvecback[pba->index_bg_rho_icdm]*y[ppw->pv->index_pt_delta_icdm]; // contribution to total perturbed stress-energy
      if (ppt->gauge == newtonian)
        ppw->rho_plus_p_theta = ppw->rho_plus_p_theta + ppw->pvecback[pba->index_bg_rho_icdm]*y[ppw->pv->index_pt_theta_icdm]; // contribution to total perturbed stress-energy

      ppw->rho_plus_p_tot += ppw->pvecback[pba->index_bg_rho_icdm];

      if (ppt->has_source_delta_m == _TRUE_) {
        delta_rho_m += ppw->pvecback[pba->index_bg_rho_icdm]*y[ppw->pv->index_pt_delta_icdm]; // contribution to delta rho_matter
        rho_m += ppw->pvecback[pba->index_bg_rho_icdm];
      }
      if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
        if (ppt->gauge == newtonian)
          rho_plus_p_theta_m += ppw->pvecback[pba->index_bg_rho_icdm]*y[ppw->pv->index_pt_theta_icdm]; // contribution to [(rho+p)theta]_matter
    ...
    /* dicdm contribution */
    if (pba->has_dicdm == _TRUE_) {
      ppw->delta_rho += ppw->pvecback[pba->index_bg_rho_dicdm]*y[ppw->pv->index_pt_delta_dicdm];
      ppw->rho_plus_p_theta += ppw->pvecback[pba->index_bg_rho_dicdm]*y[ppw->pv->index_pt_theta_dicdm];

      ppw->rho_plus_p_tot += ppw->pvecback[pba->index_bg_rho_dicdm];

      if (ppt->has_source_delta_m == _TRUE_) {
        delta_rho_m += ppw->pvecback[pba->index_bg_rho_dicdm]*y[ppw->pv->index_pt_delta_dicdm]; // contribution to delta rho_matter
        rho_m += ppw->pvecback[pba->index_bg_rho_dicdm];
      }
      if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
        rho_plus_p_theta_m += ppw->pvecback[pba->index_bg_rho_dicdm]*y[ppw->pv->index_pt_theta_dicdm]; // contribution to [(rho+p)theta]_matter
        rho_plus_p_m += ppw->pvecback[pba->index_bg_rho_dicdm];
      }
    }
    ...
    /* infer delta_cb abd theta_cb (perturbations from CDM and baryons) before adding nicdm */
    ...
    if (pba->has_nicdm == _TRUE_) {
      idx = ppw->pv->index_pt_psi0_nicdm1;
      if (ppw->approx[ppw->index_ap_nicdmfa] == (int)nicdmfa_on){
        // The perturbations are evolved integrated:
        for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++){
          rho_nicdm_bg = ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm];
          p_nicdm_bg = ppw->pvecback[pba->index_bg_p_nicdm1+n_nicdm];
          pseudo_p_nicdm = ppw->pvecback[pba->index_bg_pseudo_p_nicdm1+n_nicdm];

          rho_plus_p_nicdm = rho_nicdm_bg + p_nicdm_bg;
          w_nicdm = p_nicdm_bg/rho_nicdm_bg;
          cg2_nicdm = w_nicdm*(1.0-1.0/(3.0+3.0*w_nicdm)*(3.0*w_nicdm-2.0+pseudo_p_nicdm/p_nicdm_bg));
          if ((ppt->has_source_delta_nicdm == _TRUE_) || (ppt->has_source_theta_nicdm == _TRUE_) || (ppt->has_source_delta_m == _TRUE_)) {
            ppw->delta_nicdm[n_nicdm] = y[idx];
            ppw->theta_nicdm[n_nicdm] = y[idx+1];
            ppw->shear_nicdm[n_nicdm] = y[idx+2];
          }

          ppw->delta_rho += rho_nicdm_bg*y[idx];
          ppw->rho_plus_p_theta += rho_plus_p_nicdm*y[idx+1];
          ppw->rho_plus_p_shear += rho_plus_p_nicdm*y[idx+2];
          ppw->delta_p += cg2_nicdm*rho_nicdm_bg*y[idx];

          ppw->rho_plus_p_tot += rho_plus_p_nicdm;
        ...
        for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++){
          rho_delta_nicdm = 0.0;
          rho_plus_p_theta_nicdm = 0.0;
          rho_plus_p_shear_nicdm = 0.0;
          delta_p_nicdm = 0.0;
          factor = pba->factor_nicdm[n_nicdm]/pow(a,4);
          ...
          for (index_q=0; index_q < ppw->pv->q_size_nicdm[n_nicdm]; index_q ++) {
            ...
            q = pba->q_nicdm[n_nicdm][index_q];
            q2 = q*q;
            epsilon = sqrt(q2+pba->M_nicdm[n_nicdm]*pba->M_nicdm[n_nicdm]*a2);

            rho_delta_nicdm += q2*epsilon*pba->w_nicdm[n_nicdm][index_q]*y[idx];
            rho_plus_p_theta_nicdm += q2*q*pba->w_nicdm[n_nicdm][index_q]*y[idx+1];
            rho_plus_p_shear_nicdm += q2*q2/epsilon*pba->w_nicdm[n_nicdm][index_q]*y[idx+2];
            delta_p_nicdm += q2*q2/epsilon*pba->w_nicdm[n_nicdm][index_q]*y[idx];

            //Jump to next momentum bin:
            idx+=(ppw->pv->l_max_nicdm[n_nicdm]+1);
          }

          rho_delta_nicdm *= factor;
          rho_plus_p_theta_nicdm *= k*factor;
          rho_plus_p_shear_nicdm *= 2.0/3.0*factor;
          delta_p_nicdm *= factor/3.;

          if ((ppt->has_source_delta_nicdm == _TRUE_) || (ppt->has_source_theta_nicdm == _TRUE_) || (ppt->has_source_delta_m == _TRUE_)) {
            ppw->delta_nicdm[n_nicdm] = rho_delta_nicdm/ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm];
            ppw->theta_nicdm[n_nicdm] = rho_plus_p_theta_nicdm/
              (ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm]+ppw->pvecback[pba->index_bg_p_nicdm1+n_nicdm]);
            ppw->shear_nicdm[n_nicdm] = rho_plus_p_shear_nicdm/
              (ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm]+ppw->pvecback[pba->index_bg_p_nicdm1+n_nicdm]);
          }

          ppw->delta_rho += rho_delta_nicdm;
          ppw->rho_plus_p_theta += rho_plus_p_theta_nicdm;
          ppw->rho_plus_p_shear += rho_plus_p_shear_nicdm;
          ppw->delta_p += delta_p_nicdm;

          ppw->rho_plus_p_tot += ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm]+ppw->pvecback[pba->index_bg_p_nicdm1+n_nicdm];
        }
      }
      if (ppt->has_source_delta_m == _TRUE_) {
        for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++){
          delta_rho_m += ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm]*ppw->delta_nicdm[n_nicdm]; // contribution to delta rho_matter
          rho_m += ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm];
        }
      }
      if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
        for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++){
          rho_plus_p_theta_m += (ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm]+ppw->pvecback[pba->index_bg_p_nicdm1+n_nicdm])
            *ppw->theta_nicdm[n_nicdm]; // contribution to [(rho+p)theta]_matter
          rho_plus_p_m += (ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm]+ppw->pvecback[pba->index_bg_p_nicdm1+n_nicdm]);
        }
      }
    }
    ...
    */
    ...
    if (pba->has_idefld == _TRUE_) {
      ...
      class_call(background_w_idefld(pba,a,&w_idefld,&dw_over_da_idefld,&integral_idefld), pba->error_message, ppt->error_message);
      w_prime_idefld = dw_over_da_idefld * a_prime_over_a * a;

      if (pba->use_ppf == _FALSE_) {
        ppw->delta_rho_idefld = ppw->pvecback[pba->index_bg_rho_idefld]*y[ppw->pv->index_pt_delta_idefld];
        ppw->rho_plus_p_theta_idefld = (1.+w_idefld)*ppw->pvecback[pba->index_bg_rho_idefld]*y[ppw->pv->index_pt_theta_idefld];
        ca2_idefld = w_idefld - w_prime_idefld / 3. / (1.+w_idefld) / a_prime_over_a;
        /** We must gauge transform the pressure perturbation from the fluid rest-frame to the gauge we are working in */
        ppw->delta_p_idefld = pba->cs2_idefld * ppw->delta_rho_idefld + (pba->cs2_idefld-ca2_idefld)*(3*a_prime_over_a*ppw->rho_plus_p_theta_idefld/k/k);
      }
      else {
        s2sq = ppw->s_l[2]*ppw->s_l[2];
        c_gamma_k_H_square = pow(pba->c_gamma_over_c_idefld*k/a_prime_over_a,2)*pba->cs2_idefld;
        ...
        */
        if (c_gamma_k_H_square > ppr->c_gamma_k_H_square_max)
          ...
          Gamma_idefld = y[ppw->pv->index_pt_Gamma_idefld];

        if (ppt->gauge == synchronous){
          alpha = (y[ppw->pv->index_pt_eta]+1.5*a2/k2/s2sq*(ppw->delta_rho+3*a_prime_over_a/k2*ppw->rho_plus_p_theta)-Gamma_idefld)/a_prime_over_a;
        ...
        ppw->S_idefld = ppw->pvecback[pba->index_bg_rho_idefld]*(1.+w_idefld)*1.5*a2/k2/a_prime_over_a*
          (ppw->rho_plus_p_theta/ppw->rho_plus_p_tot+k2*alpha);
        // note that the last terms in the ratio do not include idefld, that's correct, it's the whole point of the PPF scheme
        /** We must now check the stiffness criterion again and set Gamma_prime_idefld accordingly. */
        if (c_gamma_k_H_square > ppr->c_gamma_k_H_square_max){
          ppw->Gamma_prime_idefld = 0.;
        }
        else{
          ppw->Gamma_prime_idefld = a_prime_over_a*(ppw->S_idefld/(1.+c_gamma_k_H_square) - (1.+c_gamma_k_H_square)*Gamma_idefld);
        }
        Gamma_prime_plus_a_prime_over_a_Gamma = ppw->Gamma_prime_idefld+a_prime_over_a*Gamma_idefld;
        // delta and theta in both gauges gauge:
        ppw->rho_plus_p_theta_idefld = ppw->pvecback[pba->index_bg_rho_idefld]*(1.+w_idefld)*ppw->rho_plus_p_theta/ppw->rho_plus_p_tot-
          k2*2./3.*a_prime_over_a/a2/(1+4.5*a2/k2/s2sq*ppw->rho_plus_p_tot)*
          (ppw->S_idefld-Gamma_prime_plus_a_prime_over_a_Gamma/a_prime_over_a);
        ppw->delta_rho_idefld = -2./3.*k2*s2sq/a2*Gamma_idefld-3*a_prime_over_a/k2*ppw->rho_plus_p_theta_idefld;

        /** Now construct the pressure perturbation, see 1903.xxxxx. */
        /** Construct energy density and pressure for DE (_idefld) and the rest (_t).
            Also compute derivatives. */
        rho_idefld = ppw->pvecback[pba->index_bg_rho_idefld];
        p_idefld = w_idefld*rho_idefld;
        rho_idefld_prime = -3*a_prime_over_a*(rho_idefld+p_idefld);
        p_idefld_prime = w_prime_idefld*rho_idefld-3*a_prime_over_a*(1+w_idefld)*p_idefld;
        rho_t = ppw->pvecback[pba->index_bg_rho_tot] - rho_idefld;
        p_t = ppw->pvecback[pba->index_bg_p_tot] - p_idefld;
        rho_t_prime = -3*a_prime_over_a*(rho_t+p_t);
        p_t_prime = ppw->pvecback[pba->index_bg_p_tot_prime]-p_idefld_prime;
        ...
        S = ppw->S_idefld;
        S_prime = -Z_prime/Z*S+1./Z*(rho_idefld_prime+p_idefld_prime)*(theta_t+k2*alpha)+1./Z*(rho_idefld+p_idefld)*(theta_t_prime+k2*alpha_prime);
        /** Analytic derivative of the equation for ppw->rho_plus_p_theta_idefld above. */
        rho_plus_p_theta_idefld_prime = Z_prime*(S-1./(1.+Y)*(S/(1.+1./X)+Gamma_idefld*X)) +
          Z*(S_prime + Y_prime/(1.+Y*Y+2*Y)*(S/(1.+1./X)+Gamma_idefld*X)-
             1./(1.+Y)*(S_prime/(1.+1./X)+S*X_prime/(1.+X*X+2*X)+ppw->Gamma_prime_idefld*X+Gamma_idefld*X_prime))-
          k2*alpha_prime*(rho_idefld+p_idefld)-k2*alpha*(rho_idefld_prime+p_idefld_prime);

        /** We can finally compute the pressure perturbation using the Euler equation for theta_idefld */
        ppw->delta_p_idefld = (rho_plus_p_theta_idefld_prime+4*a_prime_over_a* ppw->rho_plus_p_theta_idefld - (rho_idefld+p_idefld)*metric_euler)/k2;
      }

      ppw->delta_rho += ppw->delta_rho_idefld;
      ppw->rho_plus_p_theta += ppw->rho_plus_p_theta_idefld;
      ppw->delta_p += ppw->delta_p_idefld;

      ppw->rho_plus_p_tot += (1.+w_idefld)*ppw->pvecback[pba->index_bg_rho_idefld];
  ...
  if (_tensors_) {
    ...
    if (ppt->evolve_tensor_ur == _TRUE_){
      ...
      if (ppt->tensor_method == tm_massless_approximation) {
        ...
        if (pba->has_nicdm == _TRUE_) {
          for (n_nicdm = 0; n_nicdm < pba->N_nicdm; n_nicdm++) {
            /* (3 p_nicdm1) is the "relativistic" contribution to rho_nicdm1 */
            rho_relativistic += 3.*ppw->pvecback[pba->index_bg_p_nicdm1+n_nicdm];
          }
        }
      }
    ...
    /** - --> nicdm contribution to gravitational wave source: */
    if (ppt->evolve_tensor_nicdm == _TRUE_){
      ...
      for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++){
        ...
        for (index_q=0; index_q < ppw->pv->q_size_nicdm[n_nicdm]; index_q ++) {
          ...
          gwnicdm += q2*q2/epsilon*pba->w_nicdm[n_nicdm][index_q]*(1./15.*y[idx]+2./21.*y[idx+2]+1./35.*y[idx+4]);

          //Jump to next momentum bin:
          idx+=(ppw->pv->l_max_nicdm[n_nicdm]+1);
        }

        gwnicdm *= -_SQRT6_*4*a2*factor;

        ppw->gw_source += gwnicdm;
...
int perturbations_sources(
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == synchronous) {
      ...
      /* icdm is always on in synchronous gauge, see error message above that checks gauge and has_icdm */
      if (ppt->has_source_h == _TRUE_)
        _set_source_(ppt->index_tp_h) = - 2 * y[ppw->pv->index_pt_delta_icdm];
    ...
    /* delta_dicdm */
    if (ppt->has_source_delta_dicdm == _TRUE_) {
      _set_source_(ppt->index_tp_delta_dicdm) = y[ppw->pv->index_pt_delta_dicdm]
        + (3.*a_prime_over_a+a*pba->Gamma_dicdm)*theta_over_k2; // N-body gauge correction;
    }

    /* delta_idefld */
    if (ppt->has_source_delta_idefld == _TRUE_) {
      _set_source_(ppt->index_tp_delta_idefld) = ppw->delta_rho_idefld/pvecback[pba->index_bg_rho_idefld]
        + 3.*a_prime_over_a*(1.+pvecback[pba->index_bg_w_idefld])*theta_over_k2; // N-body gauge correction
    }
    ...
    /* delta_nicdm1 */
    if (ppt->has_source_delta_nicdm == _TRUE_) {
      for (index_tp = ppt->index_tp_delta_nicdm1; index_tp < ppt->index_tp_delta_nicdm1+pba->N_nicdm; index_tp++) {
        _set_source_(index_tp) = ppw->delta_nicdm[index_tp - ppt->index_tp_delta_nicdm1]
          + 3.*a_prime_over_a*(1+pvecback[index_tp - ppt->index_tp_delta_nicdm1 + pba->index_bg_p_nicdm1]
                               /pvecback[index_tp - ppt->index_tp_delta_nicdm1 + pba->index_bg_rho_nicdm1])*theta_over_k2; // N-body gauge correction
      }
    }
    ...
    /* theta_dicdm */
    if (ppt->has_source_theta_dicdm == _TRUE_) {
      _set_source_(ppt->index_tp_theta_dicdm) = y[ppw->pv->index_pt_theta_dicdm]
        + theta_shift; // N-body gauge correction
    }

    /* theta_idefld */
    if (ppt->has_source_theta_idefld == _TRUE_) {

      class_call(background_w_idefld(pba,a,&w_idefld,&dw_over_da_idefld,&integral_idefld), pba->error_message, ppt->error_message);

      _set_source_(ppt->index_tp_theta_idefld) = ppw->rho_plus_p_theta_idefld/(1.+w_idefld)/pvecback[pba->index_bg_rho_idefld]
        + theta_shift; // N-body gauge correction
    }
    ...
    /* theta_nicdm1 */
    if (ppt->has_source_theta_nicdm == _TRUE_) {
      for (index_tp = ppt->index_tp_theta_nicdm1; index_tp < ppt->index_tp_theta_nicdm1+pba->N_nicdm; index_tp++) {
        _set_source_(index_tp) = ppw->theta_nicdm[index_tp - ppt->index_tp_theta_nicdm1]
          + theta_shift; // N-body gauge correction
      }
    }
  }
...
int perturbations_print_variables(double tau,
  ...
  double delta_dicdm=0.,theta_dicdm=0.;
  ...
  /** - nicdm sector begins */
  int n_nicdm;
  double *delta_nicdm=NULL, *theta_nicdm=NULL, *shear_nicdm=NULL, *delta_p_over_delta_rho_nicdm=NULL;
  double rho_nicdm_bg, p_nicdm_bg, pseudo_p_nicdm, w_nicdm;
  double rho_delta_nicdm = 0.0;
  double rho_plus_p_theta_nicdm = 0.0;
  double rho_plus_p_shear_nicdm = 0.0;
  double delta_p_nicdm = 0.0;
  double factor = 0.0;
  double q,q2,epsilon;
  /** - nicdm sector ends */
  ...
  if (pba->has_nicdm == _TRUE_){
    class_alloc(delta_nicdm, sizeof(double)*pba->N_nicdm,error_message);
    class_alloc(theta_nicdm, sizeof(double)*pba->N_nicdm,error_message);
    class_alloc(shear_nicdm, sizeof(double)*pba->N_nicdm,error_message);
    class_alloc(delta_p_over_delta_rho_nicdm, sizeof(double)*pba->N_nicdm,error_message);
  }
  ...
  if (_scalars_) {
    ...
    if (pba->has_nicdm == _TRUE_) {
      ...
      if (ppw->approx[ppw->index_ap_nicdmfa] == (int)nicdmfa_on){
        // The perturbations are evolved integrated:
        for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++){
          rho_nicdm_bg = pvecback[pba->index_bg_rho_nicdm1+n_nicdm];
          p_nicdm_bg = pvecback[pba->index_bg_p_nicdm1+n_nicdm];
          pseudo_p_nicdm = pvecback[pba->index_bg_pseudo_p_nicdm1+n_nicdm];
          w_nicdm = p_nicdm_bg/rho_nicdm_bg;

          delta_nicdm[n_nicdm] = y[idx];
          theta_nicdm[n_nicdm] = y[idx+1];
          shear_nicdm[n_nicdm] = y[idx+2];
          //This is the adiabatic sound speed:
          delta_p_over_delta_rho_nicdm[n_nicdm] = w_nicdm*(1.0-1.0/(3.0+3.0*w_nicdm)*(3.0*w_nicdm-2.0+pseudo_p_nicdm/p_nicdm_bg));
          idx += ppw->pv->l_max_nicdm[n_nicdm]+1;
        }
      }
      else{
        ...
        for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++){
          ...
          delta_nicdm[n_nicdm] = rho_delta_nicdm/ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm];
          theta_nicdm[n_nicdm] = rho_plus_p_theta_nicdm/
            (ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm]+ppw->pvecback[pba->index_bg_p_nicdm1+n_nicdm]);
          shear_nicdm[n_nicdm] = rho_plus_p_shear_nicdm/
            (ppw->pvecback[pba->index_bg_rho_nicdm1+n_nicdm]+ppw->pvecback[pba->index_bg_p_nicdm1+n_nicdm]);
          delta_p_over_delta_rho_nicdm[n_nicdm] = delta_p_nicdm/rho_delta_nicdm;
    ...
    if (pba->has_dicdm == _TRUE_) {
      ...
      delta_dicdm = y[ppw->pv->index_pt_delta_dicdm];
      theta_dicdm = y[ppw->pv->index_pt_theta_dicdm];
    ...
    if (ppt->gauge == synchronous) {
      ...
      if (pba->has_dr == _TRUE_) {
        delta_dr += (-4.*a*H+a*pba->Gamma_dicdm*pvecback[pba->index_bg_rho_dicdm]/pvecback[pba->index_bg_rho_dr])*alpha;
      ...
      if (pba->has_nicdm == _TRUE_) {
        for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++){
          /** - --> TODO: gauge transformation of delta, deltaP/rho (?) and theta using -= 3aH(1+w_nicdm) alpha for delta. */
        }
      }

      if (pba->has_dicdm == _TRUE_) {
        delta_dicdm += alpha*(-a*pba->Gamma_dicdm-3.*a*H);
        theta_dicdm += k*k*alpha;
      }
    ...
    if ((pba->has_nicdm == _TRUE_) && ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_))) {
      for (n_nicdm=0; n_nicdm < pba->N_nicdm; n_nicdm++){
        class_store_double(dataptr, delta_nicdm[n_nicdm], _TRUE_, storeidx);
        class_store_double(dataptr, theta_nicdm[n_nicdm], _TRUE_, storeidx);
        class_store_double(dataptr, shear_nicdm[n_nicdm], _TRUE_, storeidx);
        class_store_double(dataptr, delta_p_over_delta_rho_nicdm[n_nicdm],  _TRUE_, storeidx);
      }
    }
    /* Decaying cold dark matter */
    class_store_double(dataptr, delta_dicdm, pba->has_dicdm, storeidx);
    class_store_double(dataptr, theta_dicdm, pba->has_dicdm, storeidx);
    /* Decay radiation */
    ...
    /** Fluid */
    class_store_double(dataptr, ppw->delta_rho_idefld, pba->has_idefld, storeidx);
    class_store_double(dataptr, ppw->rho_plus_p_theta_idefld, pba->has_idefld, storeidx);
    class_store_double(dataptr, ppw->delta_p_idefld, pba->has_idefld, storeidx);
    //fprintf(ppw->perturbations_output_file,"\n");
  ...
  if (pba->has_nicdm == _TRUE_){
    ...
    free(theta_nicdm);
    free(shear_nicdm);
    free(delta_p_over_delta_rho_nicdm);
  }
...
int perturbations_derivs(double tau,
  ...
  /* for use with fluid (idefld): */
  double w_idefld,dw_over_da_idefld,w_prime_idefld,integral_idefld;
  ...
  /* for use with non-cold dark matter (nicdm): */
  int index_q,n_nicdm,idx;
  double q,epsilon,dlnf0_dlnq,qk_div_epsilon;
  double rho_nicdm_bg,p_nicdm_bg,pseudo_p_nicdm,w_nicdm,ca2_nicdm,ceff2_nicdm=0.,cvis2_nicdm=0.;

  /* for use with curvature */
  double cotKgen, sqrt_absK;
  double s2_squared, ssqrt3;

  /* for use with dicdm and dr */
  ...
  if (_scalars_) {
    ...
    if (pba->has_icdm == _TRUE_) {
      ...
      /** - ----> newtonian gauge: icdm density and velocity */

      if (ppt->gauge == newtonian) {
        dy[pv->index_pt_delta_icdm] = -(y[pv->index_pt_theta_icdm]+metric_continuity); /* icdm density */

        dy[pv->index_pt_theta_icdm] = - a_prime_over_a*y[pv->index_pt_theta_icdm] + metric_euler; /* icdm velocity */
      }

      /** - ----> synchronous gauge: icdm density only (velocity set to zero by definition of the gauge) */

      if (ppt->gauge == synchronous) {
        dy[pv->index_pt_delta_icdm] = -metric_continuity
		+xhi_idecdm*a_prime_over_a*pvecback[pba->index_bg_rho_idefld]/pvecback[pba->index_bg_rho_icdm](y[pv->index_pt_delta_idefld]-y[pv->index_pt_delta_icdm])
	       +xhi_idecdm*a_prime_over_a*pvecback[pba->index_bg_rho_idefld]/pvecback[pba->index_bg_rho_icdm]*total-expansionrate	; /* icdm density */
      }
    }
    ...
    if (pba->has_dicdm == _TRUE_) {
      ...
      dy[pv->index_pt_delta_dicdm] = -(y[pv->index_pt_theta_dicdm]+metric_continuity)
        - a * pba->Gamma_dicdm / k2 * metric_euler; /* dicdm density */

      dy[pv->index_pt_theta_dicdm] = - a_prime_over_a*y[pv->index_pt_theta_dicdm] + metric_euler; /* dicdm velocity */
    }

    /** - ---> dr */

    if ((pba->has_dicdm == _TRUE_)&&(pba->has_dr == _TRUE_)) {
      ...
      */
      ...
      fprime_dr = pba->Gamma_dicdm*pvecback[pba->index_bg_rho_dicdm]*pow(a,5)/pow(pba->H0,2);

      /** - ----> dr F0 */
      dy[pv->index_pt_F0_dr] = -k*y[pv->index_pt_F0_dr+1]-4./3.*metric_continuity*f_dr+
        fprime_dr*(y[pv->index_pt_delta_dicdm]+metric_euler/k2);

      /** - ----> dr F1 */
      dy[pv->index_pt_F0_dr+1] = k/3.*y[pv->index_pt_F0_dr]-2./3.*k*y[pv->index_pt_F0_dr+2]*s2_squared +
        4*metric_euler/(3.*k)*f_dr + fprime_dr/k*y[pv->index_pt_theta_dicdm];
    ...
    if (pba->has_idefld == _TRUE_) {
      ...
      if (pba->use_ppf == _FALSE_){
        ...
        ca2 = w_idefld - w_prime_idefld / 3. / (1.+w_idefld) / a_prime_over_a;
        cs2 = pba->cs2_idefld;

        /** - ----> fluid density */

        dy[pv->index_pt_delta_idefld] =
          -(1+w_idefld)*(y[pv->index_pt_theta_idefld]+metric_continuity)
          -3.*(1-w_idefld)*a_prime_over_a*(y[pv->index_pt_delta_idefld]-(3.*(1+w_idefld)+xhi_ideicdm)*a_prime_over_a*a_prime_over_a*y[pv->index_pt_theta_idefld]/k2)-xhi_ideicdm*aprime_over_a*total_expansionrate;

          
        /** - ----> fluid velocity */

        dy[pv->index_pt_theta_idefld] = /* fluid velocity */
          2*a_prime_over_a*y[pv->index_pt_theta_idefld]
          +k2/(1.+w_idefld)*y[pv->index_pt_delta_idefld]
	  2.*a_prime_over_a*xhi_ideicdm/(1.+w_idefld)*y[pv->index_pt_theta_idefld]
	  -xhi_ideicdm*a_prime_over_a/(1.+w_idefld)*y[pv->index_pt_theta_icdm]
          +metric_euler;
      }
      else {
        dy[pv->index_pt_Gamma_idefld] = ppw->Gamma_prime_idefld; /* Gamma variable of PPF formalism */
      }
    ...
    /** - ---> non-cold dark matter (nicdm): massive neutrinos, WDM, etc. */
    //TBC: curvature in all nicdm
    if (pba->has_nicdm == _TRUE_) {

      idx = pv->index_pt_psi0_nicdm1;

      /** - ----> first case: use a fluid approximation (nicdmfa) */
      //TBC: curvature
      if (ppw->approx[ppw->index_ap_nicdmfa] == (int)nicdmfa_on) {

        /** - -----> loop over species */

        for (n_nicdm=0; n_nicdm<pv->N_nicdm; n_nicdm++) {

          /** - -----> define intermediate quantitites */

          rho_nicdm_bg = pvecback[pba->index_bg_rho_nicdm1+n_nicdm]; /* background density */
          p_nicdm_bg = pvecback[pba->index_bg_p_nicdm1+n_nicdm]; /* background pressure */
          pseudo_p_nicdm = pvecback[pba->index_bg_pseudo_p_nicdm1+n_nicdm]; /* pseudo-pressure (see CLASS IV paper) */
          w_nicdm = p_nicdm_bg/rho_nicdm_bg; /* equation of state parameter */
          ca2_nicdm = w_nicdm/3.0/(1.0+w_nicdm)*(5.0-pseudo_p_nicdm/p_nicdm_bg); /* adiabatic sound speed */
          ...
          if (ppr->nicdm_fluid_approximation == nicdmfa_mb) {
            ceff2_nicdm = ca2_nicdm;
            cvis2_nicdm = 3.*w_nicdm*ca2_nicdm;
          }
          if (ppr->nicdm_fluid_approximation == nicdmfa_hu) {
            ceff2_nicdm = ca2_nicdm;
            cvis2_nicdm = w_nicdm;
          }
          if (ppr->nicdm_fluid_approximation == nicdmfa_CLASS) {
            ceff2_nicdm = ca2_nicdm;
            cvis2_nicdm = 3.*w_nicdm*ca2_nicdm;
          }
          ...
          dy[idx] = -(1.0+w_nicdm)*(y[idx+1]+metric_continuity)-
            3.0*a_prime_over_a*(ceff2_nicdm-w_nicdm)*y[idx];

          /** - -----> exact euler equation */

          dy[idx+1] = -a_prime_over_a*(1.0-3.0*ca2_nicdm)*y[idx+1]+
            ceff2_nicdm/(1.0+w_nicdm)*k2*y[idx]-k2*y[idx+2]
            + metric_euler;
          ...
          if (ppr->nicdm_fluid_approximation == nicdmfa_mb) {
            ...
            dy[idx+2] = -3.0*(a_prime_over_a*(2./3.-ca2_nicdm-pseudo_p_nicdm/p_nicdm_bg/3.)+1./tau)*y[idx+2]
              +8.0/3.0*cvis2_nicdm/(1.0+w_nicdm)*s_l[2]*(y[idx+1]+metric_shear);

          }

          if (ppr->nicdm_fluid_approximation == nicdmfa_hu) {

            dy[idx+2] = -3.0*a_prime_over_a*ca2_nicdm/w_nicdm*y[idx+2]
              +8.0/3.0*cvis2_nicdm/(1.0+w_nicdm)*s_l[2]*(y[idx+1]+metric_shear);
          ...
          if (ppr->nicdm_fluid_approximation == nicdmfa_CLASS) {
            ...
            dy[idx+2] = -3.0*(a_prime_over_a*(2./3.-ca2_nicdm-pseudo_p_nicdm/p_nicdm_bg/3.)+1./tau)*y[idx+2]
              +8.0/3.0*cvis2_nicdm/(1.0+w_nicdm)*s_l[2]*(y[idx+1]+metric_ufa_class);

          }

          /** - -----> jump to next species */

          idx += pv->l_max_nicdm[n_nicdm]+1;
        }
      }
      ...
      /** - ----> second case: use exact equation (Boltzmann hierarchy on momentum grid) */
        ...
        for (n_nicdm=0; n_nicdm<pv->N_nicdm; n_nicdm++) {
          ...
          for (index_q=0; index_q < pv->q_size_nicdm[n_nicdm]; index_q++) {

            /** - -----> define intermediate quantities */

            dlnf0_dlnq = pba->dlnf0_dlnq_nicdm[n_nicdm][index_q];
            q = pba->q_nicdm[n_nicdm][index_q];
            epsilon = sqrt(q*q+a2*pba->M_nicdm[n_nicdm]*pba->M_nicdm[n_nicdm]);
            qk_div_epsilon = k*q/epsilon;

            /** - -----> nicdm density for given momentum bin */

            dy[idx] = -qk_div_epsilon*y[idx+1]+metric_continuity*dlnf0_dlnq/3.;

            /** - -----> nicdm velocity for given momentum bin */

            dy[idx+1] = qk_div_epsilon/3.0*(y[idx] - 2*s_l[2]*y[idx+2])
              -epsilon*metric_euler/(3*q*k)*dlnf0_dlnq;

            /** - -----> nicdm shear for given momentum bin */

            dy[idx+2] = qk_div_epsilon/5.0*(2*s_l[2]*y[idx+1]-3.*s_l[3]*y[idx+3])
              -s_l[2]*metric_shear*2./15.*dlnf0_dlnq;

            /** - -----> nicdm l>3 for given momentum bin */

            for (l=3; l<pv->l_max_nicdm[n_nicdm]; l++){
              dy[idx+l] = qk_div_epsilon/(2.*l+1.0)*(l*s_l[l]*y[idx+(l-1)]-(l+1.)*s_l[l+1]*y[idx+(l+1)]);
            }

            /** - -----> nicdm lmax for given momentum bin (truncation as in Ma and Bertschinger)
    ...
    if (ppt->evolve_tensor_nicdm == _TRUE_) {
      ...
      for (n_nicdm=0; n_nicdm<pv->N_nicdm; n_nicdm++) {
        ...
        for (index_q=0; index_q < pv->q_size_nicdm[n_nicdm]; index_q++) {
          ...
          /** - ----> nicdm l>0 for given momentum bin */

          for (l=1; l<pv->l_max_nicdm[n_nicdm]; l++){
            dy[idx+l] = qk_div_epsilon/(2.*l+1.0)*(l*s_l[l]*y[idx+(l-1)]-(l+1.)*s_l[l+1]*y[idx+(l+1)]);
          }
