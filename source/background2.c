...
int background_functions(
  ...
  double w_idefld, dw_over_da, integral_idefld;
  ...
  if (pba->has_cdm == _TRUE_) {
    pvecback[pba->index_bg_rho_cdm] = pba->Omega0_cdm * pow(pba->H0,2) / pow(a,3) + pba->Omega0_idefld * pow(pba->H0,2) / pow(a,3) * ( (xhi_idecdm / (3* w_idefld +xhi_idecdm ) ) * (1- pow(a,-3*w_idefld-xhi_idecdm)));
  ...
  if (pba->has_idefld == _TRUE_) {
    /* get w_idefld from dedicated function */
    class_call(background_w_idefld(pba,a,&w_idefld,&dw_over_da,&integral_idefld), pba->error_message, pba->error_message);
    pvecback[pba->index_bg_w_idefld] = w_idefld;
    
    
    /* get rho_idefld from vector of integrated variables */
    pvecback[pba->index_bg_rho_idefld] = pba->Omega0_idefld * pow(pba->H0,2) /  pow(a,3*w_idefld+3+xhi_idecdm);
    ...
    // pvecback[pba->index_bg_rho_idefld] = pba->Omega0_idefld * pow(pba->H0,2) / pow(a,3.*(1.+pba->w0_idefld+pba->wa_idefld)) * exp(3.*pba->wa_idefld*(a-1.));
    // But now everthing is integrated numerically for a given w_idefld(a) defined in the function background_w_idefld.

    rho_tot += pvecback[pba->index_bg_rho_idefld];
    p_tot += w_idefld * pvecback[pba->index_bg_rho_idefld];
    dp_dloga += (a*dw_over_da-3*(1+w_idefld)*w_idefld)*pvecback[pba->index_bg_rho_idefld];
  }
 ...
 * @param w_idefld          Output: equation of state parameter w_idefld(a)
 * @param dw_over_da_idefld Output: function dw_idefld/da
 * @param integral_idefld   Output: function \f$ \int_{a}^{a_0} da 3(1+w_{idefld})/a \f$
 * @return the error status
 */

int background_w_idefld(
                     struct background * pba,
                     double a,
                     double * w_idefld,
                     double * dw_over_da_idefld,
                     double * integral_idefld
                     ) {
  ...
  case CLP:
    *w_idefld = pba->w0_idefld + pba->wa_idefld * (1. - a);
    break;
  case EDE:
    // Omega_ede(a) taken from eq. (10) in 1706.00730
    Omega_ede = (pba->Omega0_idefld - pba->Omega_EDE*(1.-pow(a,-3.*pba->w0_idefld)))
      /(pba->Omega0_idefld+(1.-pba->Omega0_idefld)*pow(a,3.*pba->w0_idefld))
      + pba->Omega_EDE*(1.-pow(a,-3.*pba->w0_idefld));

    // d Omega_ede / d a taken analytically from the above
    dOmega_ede_over_da = - pba->Omega_EDE* 3.*pba->w0_idefld*pow(a,-3.*pba->w0_idefld-1.)/(pba->Omega0_idefld+(1.-pba->Omega0_idefld)*pow(a,3.*pba->w0_idefld))
      - (pba->Omega0_idefld - pba->Omega_EDE*(1.-pow(a,-3.*pba->w0_idefld)))*(1.-pba->Omega0_idefld)*3.*pba->w0_idefld*pow(a,3.*pba->w0_idefld-1.)/pow(pba->Omega0_idefld+(1.-pba->Omega0_idefld)*pow(a,3.*pba->w0_idefld),2)
      + pba->Omega_EDE*3.*pba->w0_idefld*pow(a,-3.*pba->w0_idefld-1.);
    ...
    *w_idefld = - dOmega_ede_over_da*a/Omega_ede/3./(1.-Omega_ede)+a_eq/3./(a+a_eq);
    break;
  }
  ...
  switch (pba->fluid_equation_of_state) {
  case CLP:
    *dw_over_da_idefld = - pba->wa_idefld;
    break;
  case EDE:
    d2Omega_ede_over_da2 = 0.;
    *dw_over_da_idefld = - d2Omega_ede_over_da2*a/3./(1.-Omega_ede)/Omega_ede
  ...
  /** - finally, give the analytic solution of the following integral:
      \f$ \int_{a}^{a0} da 3(1+w_{idefld})/a \f$. This is used in only
  ...
  switch (pba->fluid_equation_of_state) {
  case CLP:
    *integral_idefld = 3.*((1.+pba->w0_idefld+pba->wa_idefld)*log(1./a) + pba->wa_idefld*(a-1.));
    break;
  case EDE:
  ...
  /** note: of course you can generalise these formulas to anything,
      defining new parameters pba->w..._idefld. Just remember that so
...
int background_indices(
  ...
  if (pba->Omega0_idefld != 0.)
    pba->has_idefld = _TRUE_;
  ...
  /* - index for fluid */
  class_define_index(pba->index_bg_rho_idefld,pba->has_idefld,index_bg,1);
  class_define_index(pba->index_bg_w_idefld,pba->has_idefld,index_bg,1);
  ...
  class_define_index(pba->index_bi_rho_idefld,pba->has_idefld,index_bi,1);
...
int background_checks(
  ...
  if (pba->has_idefld == _TRUE_) {
    ...
    class_call(background_w_idefld(pba,0.,&w_idefld,&dw_over_da,&integral_idefld), pba->error_message, pba->error_message);

    class_test(w_idefld >= 1./3.,
               pba->error_message,
...
int background_solve(
  ...
  /* Compute the density fraction of non-free-streaming matter (in the minimal LambdaCDM model, this would be just Omega_b + Omega_cdm). This definition takes into account interating, decaying and warm dark matter, but it would need to be refined if some part of the matter component was modelled by the fluid (idefld) or the scalar field (scf). */
...
int background_initial_conditions(
  ...
  */
  ...
  if (pba->has_idefld == _TRUE_) {
    ...
    /* rho_idefld today */
    rho_idefld_today = pba->Omega0_idefld * pow(pba->H0,2);

    /* integrate rho_idefld(a) from a_ini to a_0, to get rho_idefld(a_ini) given rho_idefld(a0) */
    class_call(background_w_idefld(pba,a,&w_idefld,&dw_over_da_idefld,&integral_idefld), pba->error_message, pba->error_message);

    /* Note: for complicated w_idefld(a) functions with no simple
       analytic integral, this is the place were you should compute
       numerically the simple 1d integral [int_{a_ini}^{a_0} 3
       [(1+w_idefld)/a] da] (e.g. with the Romberg method?) instead of
       calling background_w_idefld */

    /* rho_idefld at initial time */
    pvecback_integration[pba->index_bi_rho_idefld] = rho_idefld_today * exp(integral_idefld);
...
int background_output_titles(
  ...
  class_store_columntitle(titles,"(.)rho_idefld",pba->has_idefld);
  class_store_columntitle(titles,"(.)w_idefld",pba->has_idefld);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_idefld],pba->has_idefld,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_w_idefld],pba->has_idefld,storeidx);
...
int background_derivs(
  ...
  if (pba->has_idefld == _TRUE_) {
    /** - Compute idefld density \f$ d\rho/dloga = -3 (1+w_{idefld}(a)) \rho \f$ */
    dy[pba->index_bi_rho_idefld] = -3.*(1.+pvecback[pba->index_bg_w_idefld])*y[pba->index_bi_rho_idefld];
  }
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    if ((pba->has_lambda == _TRUE_) || (pba->has_idefld == _TRUE_) || (pba->has_scf == _TRUE_) || (pba->has_curvature == _TRUE_)) {
    ...
    if (pba->has_idefld == _TRUE_) {
      class_print_species("Dark Energy Fluid",idefld);
      budget_other+=pba->Omega0_idefld;
    }
    if (pba->has_scf == _TRUE_) {
