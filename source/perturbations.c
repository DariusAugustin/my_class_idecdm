...
random_num2=rand();
  ...
  printf("%i %i The value of has_fld is %d\n",random_num1,random_num2, pba->has_fld);
  printf("%i %i The value of pv->index_pt_theta_cdm is %i\n",random_num1,random_num2, ppw->pv->index_pt_theta_cdm);
   printf("%i %i The value of pv->index_pt_theta_fld is %i\n",random_num1,random_num2, pv->index_pt_theta_fld);
        
int incrementortest,fibutest=1;
  for (incrementortest = 0; incrementortest <= 15; incrementortest++) {
       printf("The value of y[%d] is %f\n", incrementortest, y[incrementortest]);
    fibutest+=fibutest;
   }
   printf("The value of fibutest is %i\n", fibutest);*/
  ...
  if (_scalars_) {
    ...
    if (pba->has_cdm == _TRUE_) {
      ...
      if (ppt->gauge == synchronous) {
        /**printf("test\n");
        printf("The value of pv->index_pt_delta_cdm is %i\n", pv->index_pt_delta_cdm);
        printf("The value of y[pv->index_pt_delta_cdm] is %i\n", y[pv->index_pt_delta_cdm]);
        printf("The value of pv->index_pt_delta_fld is %i\n", pv->index_pt_delta_fld);
        printf("The value of y[pv->index_pt_delta_fld] is %i\n", y[pv->index_pt_delta_fld]);*/
        dy[pv->index_pt_delta_cdm] = -metric_continuity
        + pba->xhi_idecdm*a_prime_over_a *(pvecback[pba->index_bg_rho_fld]/pvecback[pba->index_bg_rho_cdm])
        * (y[pv->index_pt_delta_fld]-y[pv->index_pt_delta_cdm])
        + pba->xhi_idecdm*a_prime_over_a *(pvecback[pba->index_bg_rho_fld]/pvecback[pba->index_bg_rho_cdm]) * (y[ppt->index_tp_theta_tot]+metric_continuity)/3 
        ;
      ...
      if (pba->use_ppf == _FALSE_){
        ...
        -(1+w_fld)*(y[pv->index_pt_theta_fld]+metric_continuity) 
         -3.*(1-w_fld)*a_prime_over_a*(y[pv->index_pt_delta_fld]
          -(3.*(1+w_fld)+pba->xhi_idecdm)*a_prime_over_a*a_prime_over_a*y[pv->index_pt_theta_fld]/k2) 
-pba->xhi_idecdm*a_prime_over_a*(y[ppt->index_tp_theta_tot]+metric_continuity)/3
        ;
...
-pba->xhi_idecdm*a_prime_over_a*(y[ppt->index_tp_theta_tot]+metric_continuity)/3 */
	  ...
	  +k2/(1.+w_fld)*y[pv->index_pt_delta_fld]
          +2.*a_prime_over_a*pba->xhi_idecdm/(1.+w_fld)*y[pv->index_pt_theta_fld]
	  -pba->xhi_idecdm*a_prime_over_a/(1.+w_fld)*0
          +metric_euler 
	  ;
